...
uses
  ...
  ExtCtrls, TableInfo, StdCtrls, ToolWin,
  ComCtrls, ImgList, SqlGenerator, GraphUtil,ZConnection,
  MouseAndKeyInput,MouseInputIntf,KeyInputIntf,LCL,LCLType,LCLIntf;
...
type
  ...
  public
    ...
    property DatabaseInfo: TZConnection read FDatabaseInfo
...
uses QueryDesignerDialogU, QueryDesignerTablesU, DbType, AsSqlParser;
...
begin
  ...
  try
    try
      ...
      if ti <> nil then
      begin
        s := TSqlGenerator.Create(FDatabaseInfo.HostName,FDatabaseInfo.Database,FDatabaseInfo.User,FDatabaseInfo.Password, p,TDbUtils.DatabaseTypeFromString(FDatabaseInfo.Protocol),FDatabaseInfo.Port);
...
begin
  ...
  if TableInfoList = nil then
    TableInfoList := TTableInfos.Create(FDatabaseInfo.HostName,FDatabaseInfo.Database,FDatabaseInfo.User,FDatabaseInfo.Password,TDbUtils.DatabaseTypeFromString(FDatabaseInfo.Protocol),FDatabaseInfo.Port);
end;
...
begin
  ...
  try
    ...
    FDatabaseInfo.GetTableNames(FSchema,'',lst);
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            AddTable(FSchema,chkTables.Items[I],Point(x,y));
            lastControl := scbDesigner.Controls[scbDesigner.ControlCount-1];
