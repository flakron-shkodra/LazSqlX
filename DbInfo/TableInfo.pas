...
type
 ...
 public
    constructor Create(Server,DatabaseName,Username,Password: string;ADatabaseType:TDatabaseType;Port:Integer=0);overload;
    constructor Create(ApplicationHandle:TApplication;Server,DatabaseName,Username,Password: string;ADatabaseType:TDatabaseType;Port:Integer=0;LogList:TStrings=nil);overload;
    constructor Create(ApplicationHandle:TApplication;Server,DatabaseName,Username,Password: string;ADatabaseType:TDatabaseType;Port:Integer=0;LogList:TListBox=nil);overload;
    destructor Destroy;override;
...
constructor TTableInfos.Create(ApplicationHandle:TApplication;Server,DatabaseName,Username,Password: string;ADatabaseType:TDatabaseType;Port:Integer=0;LogList:TStrings=nil);
begin
   LogStrings := LogList;
   FApplication := ApplicationHandle;
   Create(Server,DatabaseName,Username,Password,ADatabaseType,Port);
end;

constructor TTableInfos.Create(ApplicationHandle: TApplication; Server,
  DatabaseName, Username, Password: string; ADatabaseType: TDatabaseType;
  Port: Integer; LogList: TListBox);
begin
   FLogListBox := LogList;
   FApplication := ApplicationHandle;
   Create(Server,DatabaseName,Username,Password,ADatabaseType,Port);
...
constructor TTableInfos.Create(Server,DatabaseName,Username,Password: string;ADatabaseType:TDatabaseType;Port:Integer=0);
var
 r : TResourceStream;
begin
  ...
  begin
    adoCon.Catalog:=TAsStringUtils.RemoveChars(FDatabase,['[',']']);
    adoCon.HostName:=FServer;
    adoCon.User:=Username;
    adoCon.Password:=FPassword;
    adoCon.Port:=Port;
    adoCon.LoginPrompt := False;
    dsMetaData.Catalog:=TAsStringUtils.RemoveChars(FDatabase,['[',']']);
  end;
...
procedure TTableInfos.SetDatabaseName(AValue: string);
begin
...
begin
  ...
  dsLocalHelper.Connection := adoCon;
    ...
    if FDatabaseType in [dtOracle,dtMySql] then
    begin
      try
        ...
        begin
           CI := TImportedKey.Create;
            ...
            if FDatabaseType<> dtFirebirdd then
...
begin
   try
     ...
     if DatabaseType=dtSQLite then Schema:='';
     ...
     begin
      ...
      //timestamp not needed for generation of controls or any db operation
      if dsMetaData.FieldByName('TYPE_NAME').AsString='timestamp' then
      begin
      	dsMetaData.Next;
      	Continue;
      end;
      ...
      if DatabaseType in [dtOracle,dtMySql] then
      ...
      field.FieldDbType:=FDatabaseType;
...
procedure TTableInfos.SortByHierarchy;

  procedure CheckConstraints(tblName:string; var lst:TStringList);
  var
    t:TTableInfo;
    J: Integer;
  begin
      t := TableByName(tblName);
      if t<>nil then
      begin
        if t.ExportedKeys<>nil then
        J:= t.ExportedKeys.Count;

          for J:=0 to t.ExportedKeys.Count-1 do
          begin
           if lst.IndexOf(t.ExportedKeys[J].ForeignTableName)< 0 then
           begin
             lst.Add(t.ExportedKeys[J].ForeignTableName);
             CheckConstraints(t.ExportedKeys[J].ForeignTableName,lst);
           end;
          end;
      end;
  end;
...
var
  ...
  I: Integer;
  tt:TTableInfo;
  CurrentIndexes:array of integer;
begin

  tmp := TTableInfos.Create(True);
  lst := TStringList.Create;
  ...
  try
    ...
    for I:=0 to Count-1 do
    begin
      CheckConstraints(Self[I].Tablename,lst);
    end;

    SetLength(CurrentIndexes,Count);

    for I := 0 to lst.Count-1 do
    begin
     CurrentIndexes[lst.Count-I] := IndexOf(lst[I]);
    end;

    for I:=0 to Length(CurrentIndexes)-1 do
    begin
     Self.Exchange(I,CurrentIndexes[I]);
    end;
